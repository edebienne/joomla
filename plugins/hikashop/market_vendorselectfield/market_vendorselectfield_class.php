...
class hikashopMarket_vendorselectfield {
	...
	public function display($field, $value, $map, $inside, $options = '', $test = false) {
		...
		if(in_array($field->field_table, array('item', 'order')) && empty($value) && !$app->isAdmin()) {
		...
		if($show_type == 0) {
			...
			$ret = $vendorSelectionType->displayDropdown($map, $value, false, $options, $this->prefix . @$field->field_namekey . $this->suffix);
			...
			$ret = $nameboxType->display(
				...
				array(
					...
					'delete' => ($app->isAdmin() || empty($field->field_required)),
