		...
		<thead>
			<tr>
				...
				<th class="title" style="width:8%">
					<button class="btn" style="margin:0px;" type="button" onclick="return marketAddVendorFee();">
...
?>
			<tr class="row<?php echo $k;?>">
				<td class="hika_currency"><?php
					echo @$this->currencyType->display('market[product_fee]['.$i.'][currency]', @$fee->fee_currency_id);
				?></td>
				<td class="hika_qty">
					<input size="3" type="text" name="market[product_fee][<?php echo $i;?>][quantity]" value="<?php echo @$fee->fee_min_quantity;?>" />
				</td>
				<td class="hika_price">
					<input size="5" type="text" name="market[product_fee][<?php echo $i;?>][min_price]" value="<?php echo @$fee->fee_min_price;?>" />
				</td>
				<td class="hika_price">
					<input type="hidden" name="market[product_fee][<?php echo $i;?>][id]" value="<?php echo $fee->fee_id;?>" />
					<input size="6" type="text" name="market[product_fee][<?php echo $i;?>][value]" value="<?php echo @$fee->fee_value;?>" />
				</td>
				<td class="hika_price">
					<input size="6" type="text" name="market[product_fee][<?php echo $i;?>][fixed]" value="<?php echo @$fee->fee_fixed;?>" />
				</td>
				<td class="hika_price">
					<input size="4" type="text" name="market[product_fee][<?php echo $i;?>][percent]" value="<?php echo number_format((float)@$fee->fee_percent, 2);?>" />%
				</td>
				<td align="center">
...
?>
			<tr class="row<?php echo $k;?>"  style="display:none" id="hikamarket_tpl_product_fee">
...
function marketAddVendorFee(){
	var d = document,
		...
		htmlblocks = {
			input_fee_id: "market[product_fee]["+cpt+"][id]",
			input_fee_value: "market[product_fee]["+cpt+"][value]",
			input_fee_currency: "market[product_fee]["+cpt+"][currency]",
			input_fee_percent: "market[product_fee]["+cpt+"][percent]",
			input_fee_quantity: "market[product_fee]["+cpt+"][quantity]",
			input_fee_min_price: "market[product_fee]["+cpt+"][min_price]",
			input_fee_fixed: "market[product_fee]["+cpt+"][fixed]"
		};
	window.hikamarket.dupRow('hikamarket_tpl_product_fee', htmlblocks, "market_product_fee_" + cpt);
