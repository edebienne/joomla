...
class menusmarketViewmenusmarket extends hikamarketView {
	...
	public function listing() {
		...
		$db = JFactory::getDBO();
		if(!HIKASHOP_J16) {
			$query = 'SELECT id FROM ' . hikamarket::table('components', false) . ' WHERE link=\'option=' . HIKAMARKET_COMPONENT . '\'';
			$db->setQuery($query, 0, 1);
			$filters = array(
				'(componentid = ' . $db->loadResult() . ' OR (componentid = 0 AND link LIKE \'%option='.HIKAMARKET_COMPONENT.'%\'))',
				'type = \'component\'',
				'published>-2'
			);
		} else {
		...
		if(!empty($pageInfo->search)) {
			$searchVal = '\'%' . hikamarket::getEscaped(JString::strtolower($pageInfo->search ), true) . '%\'';
