...
class pluginsmarketViewpluginsmarket extends hikamarketView {
	...
	public function listing() {
		...
		if(!HIKASHOP_J16) {
			$query = 'SELECT * FROM ' . hikamarket::table('plugins', false).' WHERE `folder` = ' . $db->Quote($group) . ' ORDER BY published DESC, ordering ASC';
		} else {
	...
	public function form() {
		...
		if(!$plugin || !method_exists($plugin, 'type')) {
			...
			if(!empty($plugin) && (!empty($plugin->id) || !empty($plugin->extension_id))) {
				if(!HIKASHOP_J16)
					$url = 'index.php?option=com_plugins&view=plugin&client=site&task=edit&cid[]=' . $plugin->id;
				else
	...
	private function loadPluginTemplate($view = '') {
		...
		if(!file_exists($path)) {
			if(!HIKASHOP_J16) {
				$path = JPATH_PLUGINS . DS . 'hikamarket' . DS . $name;
			} else {
