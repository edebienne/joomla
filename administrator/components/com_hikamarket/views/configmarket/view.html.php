...
class configmarketViewconfigmarket extends hikamarketView {
	...
	public function acl($tpl = null) {
		...
		$title_parameters = '';
		$acl_type = 'user';
		$acl_type = JRequest::getCmd('acl_type', '');
		...
		if(!empty($acl_type)) {
			...
			foreach($groups as $group) {
				...
				if($acl_type == 'vendor_options')
					$aclConfig[$group->id] = unserialize($localAclData);
				else
		...
		hikamarket::setTitle(JText::_('HIKAM_ACL'), 'acl', self::ctrl.'&task=acl'.$title_parameters);
	...
	public function language() {
		...
		if(JFile::exists($path)) {
			...
			$file->content = JFile::read(JLanguage::getLanguagePath(JPATH_ROOT) . DS . 'en-GB' . DS . 'en-GB.' . HIKAMARKET_COMPONENT . '.ini');
		}
		$override_content = '';
