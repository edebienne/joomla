...
class hikamarketFeeClass extends hikamarketClass {
	...
	public function getProduct($product, $vendor = false) {
		...
		if(is_array($product)) {
			if(is_object(reset($product))) {
				...
				$filter = 'a.product_id IN (' . implode(',', $ids) . ')';
			} else {
				$filter = 'a.product_id IN (' . implode(',', $product) . ')';
			}
		} else {
			if(is_object($product)) {
				$filter = 'a.product_id = ' . (int)$product->product_id;
			} else {
				$filter = 'a.product_id = ' . (int)$product;
			}
		}
		...
		if($vendor) {
			if(is_array($vendor)) {
				$vendorFilter[] = 'OR ((b.fee_type = \'vendor\' OR b.fee_type = \'vendor_global\') AND (a.product_vendor_id = b.fee_target_id OR b.fee_target_id IN ('.implode(',',$vendor).')))';
			} else {
				$vendorFilter[] = 'OR ((b.fee_type = \'vendor\' OR b.fee_type = \'vendor_global\') AND (a.product_vendor_id = b.fee_target_id))';
			}
			$vendorFilter[] = 'OR (b.fee_type = \'config\')';
			$vendorFilter[] = 'OR (b.fee_type = \'config_global\')';
		}

		$query = 'SELECT b.*, a.product_id, a.product_parent_id '.
				' FROM '. hikamarket::table('fee') . ' AS b '.
				' LEFT JOIN ' . hikamarket::table('shop.product') . ' AS a ON '.
				' (b.fee_type = \'product\' AND (a.product_id = b.fee_target_id OR a.product_parent_id = b.fee_target_id)) '. implode(' ',$vendorFilter).
				' WHERE '.$filter.
				' ORDER BY fee_min_quantity ASC, fee_currency_id ASC';
	...
	public function getProducts(&$products, $vendor = false) {
		...
		$vendors = null;
		if(!empty($vendor_groups) && $vendor !== false && is_array($vendor)) {
			$query = 'SELECT vendor_id, vendor_access FROM '.hikamarket::table('vendor').' WHERE vendor_id IN ('.implode(',',$vendor).')';
			$this->db->setQuery($query);
		...
		foreach($products as &$product) {
			$public_fees = array();
			foreach($fees as $fee) {
				if((($fee->fee_type == 'vendor' || $fee->fee_type == 'vendor_global') && (int)$fee->fee_target_id == (int)$product['vendor']) || ($fee->fee_type == 'product' && (int)$fee->fee_target_id == $product['id'])) {
					$product['fee'][] = $fee;
				}
