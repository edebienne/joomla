...
class hikamarketCategoryClass extends hikamarketClass {
	...
	public function frontSaveForm() {
		...
		if(hikamarket::acl('category/edit/description')) {
			$category->category_description = JRequest::getVar('category_description','','','string',JREQUEST_ALLOWRAW);

			$safeHtmlFilter = JFilterInput::getInstance(null, null, 1, 1);
			$category->category_description = $safeHtmlFilter->clean($category->category_description, 'string');
		}

		if(!hikamarket::acl('category/edit/parent') || (!hikamarket::isVendorCategory($category->category_parent_id))) {
	...
	public function toggleId($task, $value = null) {
		if($value !== null) {
			$app = JFactory::getApplication();
			if(!$app->isAdmin() && ((int)$value == 0 || !hikamarket::isVendorCategory((int)$value) || empty($this->toggle[$task]) || ( empty($this->toggleAcl[$task]) && !hikamarket::acl('category/edit/'.$task) ) || ( !empty($this->toggleAcl[$task]) && !hikamarket::acl($this->toggleAcl[$task]) ) ))
				return false;
		}
	...
	public function toggleDelete($value1 = '', $value2 = '') {
		$app = JFactory::getApplication();
		if(!$app->isAdmin() && ((int)$value1 == 0 || !hikamarket::isVendorCategory((int)$value1) || !hikamarket::acl('category/delete')))
			return false;
	...
	public function processView(&$view) {
		...
		if($paramsOpt < 0)
			$paramsOpt = $config->get('override_vendor_category_link', 0);
		...
		foreach($view->rows as &$row) {
			if(isset($row->category_namekey) && substr($row->category_namekey, 0, 7) == 'vendor_') {
				...
				$vendorItemId = $config ->get('vendor_default_menu', '');
