...
class hikamarketStatisticsClass extends hikamarketClass {
	...
	public function __construct($config = array()) {
		...
		if(!HIKASHOP_J30)
			$this->timeoffset = ((int)$this->joomlaConfig->getValue('config.offset'))*3600;
		else
			$this->timeoffset  = ((int)$this->joomlaConfig->get('offset'))*3600;
	...
	public function getVendor($vendor) {
		...
		$ret['orders_total_unpaid'] = array(
			...
			'query' => array(
				...
				'tables' => hikamarket::table('shop.order') . ' AS hk_order ',
				'filters' => array(
					'order_type' => 'hk_order.order_type = \'subsale\'',
					'order_vendor_id' => (($vendor_id == 0) ? 'hk_order.order_vendor_id > 1' : ('hk_order.order_vendor_id = ' . (int)$vendor_id)),
					'order_vendor_paid' => 'hk_order.order_vendor_paid = 0',
					'order_status' => ('hk_order.order_status IN ('.implode(',', $valid_order_statuses).')'),
					'order_invoice_created' => '({DATE_END} <= 0 OR hk_order.order_invoice_created <= {DATE_END})',
				),
				'group' => 'hk_order.order_currency_id'
			),
			'vendor_id' => (int)$vendor_id
		...
		$extra_list = $dispatcher->trigger('onHikamarketStatisticPluginList', array(
			$vendor_id,
			array(
	...
	public function getAjaxData($vendor_id, $name, $value) {
		...
		$dispatcher = JDispatcher::getInstance();
		$dispatcher->trigger('onVendorPanelDisplay', array(&$buttons, &$statistics));
		...
		if(!empty($vendor_statistics)) {
			...
			$vendor_statistics = unserialize(base64_decode($vendor_statistics));
	...
	public function display($data, $doText = true) {
		...
		switch($data['type']) {
			...
			case 'plugin':
				JPluginHelper::importPlugin('hikamarket');
				$dispatcher = JDispatcher::getInstance();
				$ret = $dispatcher->trigger('onHikamarketStatisticPluginDisplay', array($data));
