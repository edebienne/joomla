...
<fieldset class="adminform">
	...
	public function onBeforePluginSave(&$element, &$do, $new = false) {
		...
		if(!$app->isAdmin() && isset($element->shipping_params) && (!isset($element->shipping_params->shipping_vendor_filter) || (int)$element->shipping_vendor_id > 1))
		...
		foreach($methods as $method) {
			...
			if(!empty($plugin) && method_exists($plugin, 'shippingMethods')) {
				if(is_string($method->shipping_params) && !empty($method->shipping_params))
					$method->shipping_params = unserialize($method->shipping_params);
				$instances = $plugin->shippingMethods($method);
				if(!empty($instances)) {
					foreach($instances as $id => $instance) {
						$shipping_namekey = $method->shipping_type . '_' . $method->shipping_id . '-' . $id;
