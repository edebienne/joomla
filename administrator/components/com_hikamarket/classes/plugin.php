...
class hikamarketPluginClass extends hikamarketClass {
	...
	public function get($id, $default = '') {
		...
		if(!empty($ret->plugin_params))
			$ret->plugin_params = unserialize($ret->plugin_params);
		return $ret;
	}
	...
	public function frontSaveForm($task = '', $acl = true) {
		...
		$plugin_name = JRequest::getCmd('name', '');
		...
		foreach($process_fields as $fieldName => $field) {
			...
			if((isset($field['type']) && $field['type'] == 'wysiwyg') || (!empty($field['format']) && $field['format'] == 'text'))
				$data = JRequest::getVar($fieldName, '', '',' string', JREQUEST_ALLOWRAW);
			else if($params)
		...
		if(!$status) {
			...
			if($new)
				JRequest::setVar($plugin_type.'_id', $status);
		}
	...
	public function toggleId($task, $value = null) {
		if($value !== null) {
			$app = JFactory::getApplication();
			if(!$app->isAdmin() && ((int)$value == 0 || empty($this->toggle[$task]) || !hikamarket::acl('genericplugin/edit/'.$task) || !hikamarket::isVendorPlugin((int)$value, 'plugin') ))
				return false;
		}
	...
	public function toggleDelete($value1 = '', $value2 = '') {
		$app = JFactory::getApplication();
		if(!$app->isAdmin() && ((int)$value1 == 0 || !hikamarket::acl('genericplugin/delete') || !hikamarket::isVendorPlugin((int)$value1, 'plugin')))
			return false;
