...
class hikamarketVendorClass extends hikamarketClass {
		...
		if(empty($data[$type.'_'.$id])) {
			...
			if(!empty($vendor)) {
				if(!empty($vendor->user_params)) {
					$vendor->user_params = unserialize($vendor->user_params);
				}
	...
	public function register($shopuser = false) {
		...
		if($config->get('register_paypal_required', 0) && empty($vendor->vendor_params->paypal_email)) {
			if($config->get('register_ask_paypal', 1)) {
				$do = false;
				$app = JFactory::getApplication();
	...
	public function checkProductLimitation($vendor, $display = true) {
		...
		$vendor_params = (!empty($vendor->vendor_params) && is_string($vendor->vendor_params)) ? unserialize($vendor->vendor_params) : $vendor->vendor_params;
			...
			if($vendorPayContent == 'products') {
				...
				foreach($pay_orders as $o) {
					...
					if($config_tax) {
			 		...
			 		$order_vendor_params = is_string($o->order_vendor_params) ? unserialize($o->order_vendor_params) : $o->order_vendor_params;
