...
class hikamarketFieldClass extends hikamarketClass {
	...
	protected function shopfieldFileDownload(&$found, $name, $field_table, $field_namekey, $options) {
		if(!in_array($field_table, array('order')))
			return;
		...
		$escaped_field_namekey = (!HIKASHOP_J25) ? $this->db->nameQuote($field_namekey) : $this->db->quoteName($field_namekey);
		if($vendor_id > 1) {
		...
		if(!HIKASHOP_J30)
			$searchVal = '\'%' . $this->db->getEscaped(';vendor_order_show=1;', true) . '%\'';
		else
