...
class hikamarketMenusClass extends hikamarketClass {
	...
	public function get($id, $default = '') {
		...
		if(empty($obj->hikamarket_params)) {
			...
			if(substr($obj->link, 0, 32) == 'index.php?option=com_hikamarket&') {
	...
	public function save(&$element) {
		if(!HIKASHOP_J16) {
			$query = 'SELECT a.id FROM ' . hikamarket::table('components',false) . ' AS a WHERE a.option=\''.HIKAMARKET_COMPONENT.'\'';
	...
	public function createMenu(&$moduleOtpions, $id) {
		...
		if(empty($moduleOtpions['itemid'])) {
			...
			if(!HIKASHOP_J16) {
				...
				$element->published = 1;
				$element->name = 'Menu item for vendor listing module '.$id;
				$this->save($element);
				$this->db->setQuery('SELECT id FROM '.hikamarket::table('menu',false).' WHERE alias=\''.$element->alias.'\'');
				$moduleOtpions['itemid'] = $this->db->loadResult();
			} else {
