...
class hikamarketDiscountClass extends hikamarketClass {
	...
	public function frontSaveForm($task = '', $acl = true) {
		...
		else if($vendor_id > 1 || !hikamarket::acl('discount/edit/vendor'))
	...
	public function toggleId($task, $value = null) {
		if($value !== null) {
			$app = JFactory::getApplication();
			if(!$app->isAdmin() && ((int)$value == 0 || empty($this->toggle[$task]) || !hikamarket::acl('discount/edit/'.str_replace('discount_', '', $task)) || !hikamarket::isVendorDiscount((int)$value) ))
				return false;
		}
	...
	public function toggleDelete($value1 = '', $value2 = '') {
		$app = JFactory::getApplication();
		if(!$app->isAdmin() && ((int)$value1 == 0 || !hikamarket::acl('discount/delete') || !hikamarket::isVendorDiscount((int)$value1)))
			return false;
	...
	public function discountBlocksDisplay(&$discount, &$html) {
		...
		$ret = '
<tr id="hikashop_discount_target_vendor">
	<td class="key">'.JText::_('DISCOUNT_TARGET_VENDOR').'</td>
	<td>
...
function hikamarket_discount_setVendor(el, c) {
	...
	if(v == -2 || (c && v > 1 && v == hikamarket_discount_current_vendor && el.checked)) {
