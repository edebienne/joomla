...
class hikamarketCharacteristicClass extends hikamarketClass {
	...
	public function frontSaveForm() {
		...
		if(empty($characteristic_parent_id) || !empty($formCharacteristic)) {
			...
			if(!$new && hikamarket::acl('characteristic/values/ordering') && !empty($formData['values']) && isset($formData['values']['id']) && isset($formData['values']['ordering'])) {
				...
				JArrayHelper::toInteger($formData['values']['id']);
				JArrayHelper::toInteger($formData['values']['ordering']);
				...
				JArrayHelper::toInteger($current_ids);
		...
		if($status) {
			...
			JRequest::setVar('fail', $characteristic);
		}
		return $status;
	}
	...
	public function save(&$element) {
		...
		if($new)
			$dispatcher->trigger('onBeforeCharacteristicCreate', array(&$element, &$do));
		else
			$dispatcher->trigger('onBeforeCharacteristicUpdate', array(&$element, &$do));
		...
		if($new)
			$dispatcher->trigger('onAfterCharacteristicCreate', array(&$element));
		else
			$dispatcher->trigger('onAfterCharacteristicUpdate', array(&$element));
	...
	public function toggleDelete($value1 = '', $value2 = '') {
		...
		if(!$app->isAdmin() && ((int)$value1 == 0 || (int)$value2 == 0 || !hikamarket::isVendorCharacteristic((int)$value1, (int)$value2) || !hikamarket::acl('characteristic/delete')))
			return false;
