		...
		function preflight($type, $parent) {
			...
			if(version_compare($hikashopConfig->get('version', '1.0'), '2.6.0', '<')) {
				echo '<h1>This extension works with HikaShop 2.6.0 or newer.</h1>'.
					...
					'installation abort.';
				if($type == 'update')
					JError::raiseWarning(null, 'Cannot update HikaMarket 1.6.7 without HikaShop 2.6.0 or newer');
				else
					JError::raiseWarning(null, 'Cannot install HikaMarket 1.6.7 without HikaShop 2.6.0 or newer');

				$joomConf = JFactory::getConfig();
				$debug = $joomConf->get('debug');
				if($debug) {
					JError::raiseError(500, 'Cannot install HikaMarket 1.6.7 without HikaShop 2.6.0 or newer');
				}
				return false;
			}
