...
class updateMarketController extends hikashopBridgeController {
	...
	public function install() {
		hikamarket::setTitle(HIKAMARKET_NAME, 'install', 'update');
		...
		$this->showIframe(HIKAMARKET_UPDATEURL.'install&fromversion='.JRequest::getCmd('fromversion'));
		return false;
	}
	...
	public function update() {
		...
		hikamarket::setTitle(JText::_('UPDATE_ABOUT'), 'install', 'update');
		$bar = JToolBar::getInstance('toolbar');
		$bar->appendButton('Link', HIKAMARKET_LNAME, JText::_('HIKASHOP_CPANEL'), hikamarket::completeLink('dashboard'));
