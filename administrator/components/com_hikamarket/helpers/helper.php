...
if(!file_exists($hikashopHelperFile)) {
	JError::raiseError(500, 'HikaShop not installed ( www.hikashop.com )');
	exit;
}
...
class hikamarket {
	...
	public static function &config($market = true, $reload = false) {
		...
		if(self::$configClass === null || $reload){
			...
			if( self::$configClass === null ) die(HIKAMARKET_NAME.' config not found');
	...
	public static function getPluginController($ctrl) {
		...
		$dispatcher = JDispatcher::getInstance();
		$controllers = $dispatcher->trigger('onHikamarketPluginController', array($ctrl));
	...
	public static function getCID($field = '', $int = true) {
		$res = JRequest::getVar('cid', array(), '', 'array');
		if(!empty($res))
			$res = reset($res);
	...
	public static function setTitle($name, $picture, $link) {
		...
		$html='<a href="'.hikamarket::completeLink($link).'">'.$name.'</a>';
	...
	public static function footer() {
		...
		if(JRequest::getInt('marketbuild',0)) $text .= '<!-- 1512161548 -->';
	...
	public static function initModule() {
		static $done = false;
		$fe = JRequest::getVar('hikamarket_front_end_main', 0);
		if($done || !empty($fe))
			return true;
	...
	public static function tooltip($desc, $title = '', $image = 'tooltip.png', $name = '', $href = '', $link = 1) {
		if(HIKASHOP_J30) {
			if(strpos($name, '<img ') === false && strpos($name, '"') === false)
				return hikashop_tooltip($desc, $title, $image, $name, $href, $link);

			$ret = hikashop_tooltip($desc, $title, $image, '#MYTEXT#', $href, $link);
			return str_replace('#MYTEXT#', $name, $ret);
		}
		...
		if($vendor_id > 1 && $category_id > 0) {
			...
			if($check_extra_categories) {
				...
				if(!empty($extra_categories)) {
					...
					JArrayHelper::toInteger($extra_categories);
				}
			}
	...
	public static function isVendorOrder($order_id, $vendor_id = -1) {
		...
		if($vendor_id > 1 && $order_id > 0) {
			...
			$db = JFactory::getDBO();
			$query = 'SELECT count(a.order_id) FROM ' . hikamarket::table('shop.order') . ' AS a WHERE a.order_id = '. (int)$order_id . ' AND a.order_vendor_id = ' . (int)$vendor_id;
			$db->setQuery($query);
	...
	public static function aclTest($action, $access) {
		...
		foreach($access as $a) {
			...
			if($l > 0) {
				...
				if((!$parentBan || $parentBan < $l) && substr_compare($a, '!'.$action, 0, $l) == 0) { $parentBan = $l; continue; }
			}
		}
	...
	public static function loadJslib($name) {
		...
		switch($name) {
			case 'tooltip':
				...
				$doc->addScript(HIKAMARKET_JS.'tooltip.js');
				$doc->addStyleSheet(HIKAMARKET_CSS.'tooltip.css');
...
');
			...
			case 'dropdown':
				hikashop_loadJslib('jquery');
				$doc->addScript(HIKAMARKET_JS.'dropdown.js?v='.HIKAMARKET_RESSOURCE_VERSION);
				$doc->addStyleSheet(HIKAMARKET_CSS.'dropdown.css?v='.HIKAMARKET_RESSOURCE_VERSION);
				$doc->addScriptDeclaration('
...
');
	...
	public static function orderStatus($order_status) {
		$order_upper = JString::strtoupper(trim($order_status));
	...
	public static function getProductEditionUrl($product_id, $parameters = '') {
		...
		$url_itemid = '';
		$vendor_itemid = $config->get('vendor_default_menu', 0);
		if(!empty($vendor_itemid))
			$url_itemid = '&Itemid=' . (int)$vendor_itemid;
...
class hikamarketView extends hikashopBridgeView {
	...
	public function display($tpl = null) {
		if($this->triggerView) {
			JPluginHelper::importPlugin('hikamarket');
			$dispatcher = JDispatcher::getInstance();
			$dispatcher->trigger('onHikamarketBeforeDisplayView', array(&$this));
		}
		...
		if($this->triggerView)
			$dispatcher->trigger('onHikamarketAfterDisplayView', array(&$this));
	}
...
class hikamarketPlugin extends JPlugin {
	...
	public function onPluginConfiguration(&$elements) {
		...
		if($plugin_id == 0) {
			hikamarket::setTitle($this->title, 'plugin', 'plugins&plugin_type='.$this->type.'&task=edit&name='.$this->pluginName.'&subtask=edit');
		} else {
			hikamarket::setTitle($this->title, 'plugin', 'plugins&plugin_type='.$this->type.'&task=edit&name='.$this->pluginName.'&subtask=edit&plugin_id='.$plugin_id);
		}
	}
