...
class hikamarketToggleHelper{
	...
	public function __construct() {
		$this->token = '&'.hikamarket::getFormToken().'=1';
	}
	...
	public function toggle($id, $value, $table, $extra = null) {
		...
		if(!$jsIncluded && ($params->mode == 'pictures' || $params->mode == 'class')) {
			...
			$js = 'function hikamToggleElem(el,id,v,t,e){'."\r\n".
				...
				'var url="'.hikamarket::completeLink($this->ctrl.'&task={TASK-}&value={VALUE-}&table={TABLE-}'.$this->token,true,true).'";'."\r\n".
		...
		if($params->mode == 'pictures') {
			...
			if(JRequest::getCmd('ctrl') != 'toggle')
	...
	public function delete($lineId, $elementids, $table, $confirm = false, $text = '') {
		...
		if(!$jsIncluded) {
			...
			$js = 'function hikamDeleteElem(el,id,v,t,r){'."\r\n".
				...
				'var url="'.hikamarket::completeLink($this->ctrl.$this->extra.$this->token.'&task=delete&value={VALUE-}&table={TABLE-}', true).'";'.
				'url=url.replace("{VALUE-}",v).replace("{TABLE-}",t);'."\r\n".
				'o.xRequest(url,null,function(x,p){if(x.responseText != "1") { alert(x.responseText); return; } var e = d.getElementById(id); if(e) e.style.display="none"; else el.style.display="none";});'."\r\n".
				'}';
			$doc = JFactory::getDocument();
			$doc->addScriptDeclaration($js);
		}
		...
		if(empty($text))
			$text = '<img src="'.HIKAMARKET_IMAGES.'icon-16/delete.png"/>';
