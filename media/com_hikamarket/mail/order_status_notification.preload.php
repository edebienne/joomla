...
$url = '<a href="' . $order_url . '">' . $data->order->order_number . '</a>';
$data->order->order_url = $order_url;

$data->cart = $orderClass->loadFullOrder($data->order_id, true, false);
$data->cart->coupon = new stdClass();
$price = new stdClass();
$tax = $data->cart->order_subtotal - $data->cart->order_subtotal_no_vat + $data->order->order_discount_tax + $data->order->order_shipping_tax + $data->order_payment_tax;
...
$texts = array(
	...
	'SALE_BEGIN_MESSAGE' => JText::sprintf('MAIL_SALE_BEGIN_MESSAGE', $data->order->order_number, $customer_name, HIKASHOP_LIVE),
...
$cartFooters = array();
{
	...
	if(bccomp($data->order->order_discount_price,0,5)) {
		if($shopConfig->get('price_with_tax'))
			$t = $currencyHelper->format($data->order_discount_price * -1, $data->order->order_currency_id);
		else
	...
	$vendor_price = $data->cart->order_vendor_price;
	if($vendor_price < 0)
