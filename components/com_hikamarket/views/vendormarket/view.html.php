...
class vendormarketViewvendormarket extends HikamarketView {
	...
	public function show() {
		...
		if($config->get('image_scale', '-1') !== '-1') {
			switch((int)$config->get('image_scale')) {
				case 0:
					$image_options['image_scale'] = 'outside';
					break;
				case 1:
					$image_options['image_scale'] = 'inside';
					break;
			}
		}
	...
	public function form() {
		...
		if(!count($items)) {
			...
			$pathway->addItem($itemName, hikamarket::completeLink('vendor&task=form'));
		}
	}
	...
	public function vendorEdition($type = 'auto') {
		...
		if($type == 'vendorregister' || hikamarket::acl('vendor/edit/fields')) {
			$extraFields = array(
				...
				'user' => $this->fieldsClass->getFields('frontcomp', $user, 'user')
			);
		}
		$this->assignRef('extraFields', $extraFields);
		$this->assignRef('vendorFields', $vendorFields);
	...
	public function listing() {
		...
		$dispatcher->trigger('onBeforeVendorListingDisplay', array(&$this, &$trigger_params));
