...
<form action="<?php echo hikamarket::completeLink('product&task=galleryimage', true); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
...
hikashopGallery.callbackSelection = function(tree,id) {
	...
	if( node.value && node.name ) {
		var url = "<?php echo hikamarket::completeLink('product&task=galleryimage&id='.JRequest::getInt('id').'&cid='.@$this->cid.'&product_id='.JRequest::getInt('product_id').'&folder={FOLDER}', true, true) ;?>";
...
window.hikagallery.select = function(el, id) {
	...
	<input type="hidden" name="ctrl" value="<?php echo JRequest::getCmd('ctrl', 'product'); ?>" />
