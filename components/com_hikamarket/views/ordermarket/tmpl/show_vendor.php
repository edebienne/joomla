...
<?php
	if(hikamarket::acl('order/show/billingaddress') || hikamarket::acl('order/show/shippingaddress')) {
		...
		if(HIKASHOP_RESPONSIVE) {
			$row_class = HK_GRID_ROW;
			$cell_class = ($acl_billing && $acl_shipping) ? HK_GRID_COL_6 : HK_GRID_COL_12;
		} else if($acl_billing && $acl_shipping) {
...
?>
				<a onclick="return window.orderMgr.showProduct(this);" data-popup="<?php echo hikamarket::completeLink('shop.product&task=show&cid='.$product->product_id, true); ?>" href="<?php echo hikamarket::completeLink('shop.product&task=show&cid='.$product->product_id); ?>"><?php
...
<?php
	...
	$dispatcher->trigger('onAfterOrderProductsListingDisplay', array(&$this->order, 'order_frontvendor_vendor_show'));
?>
...
window.orderMgr.showProduct = function(el) {
	...
	var href = el.getAttribute('data-popup');
	if(!href)
