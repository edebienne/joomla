...
<form action="<?php echo hikamarket::completeLink('order&task=save&subtask=general&cid='.$this->order->order_id); ?>" method="post" name="adminForm" id="adminForm">
<dl class="hikam_options">
	...
	<dd><?php
		echo $this->order_status->display('data[order][order_status]', $this->order->order_status, 'onchange="window.orderMgr.status_changed(this);"', false, @$this->order_status_filters);
	?></dd>
</dl>
...
window.orderMgr.general_history_changed = function(el) {
	...
	<input type="hidden" name="subtask" value="general" />
