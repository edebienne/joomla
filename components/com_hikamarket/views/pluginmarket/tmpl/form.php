...
<form action="<?php echo hikamarket::completeLink('plugin&plugin_type='.$this->type);?>" method="post" name="hikamarket_form" id="hikamarket_plugin_form">
	<h1><?php echo JText::_('HIKA_PLUGIN') . $this->name; ?></h1>
...
?>
	...
	?><button class="btn btn-danger" onclick="return window.pluginMgr.cancelRestriction();"><img src="<?php echo HIKAMARKET_IMAGES; ?>icon-16/cancel.png" alt="" style="vertical-align:middle;"/> <?php echo JText::_('HIKA_CANCEL'); ?></button></div>
	<button class="btn btn-default" onclick="return window.pluginMgr.addRestriction();"><img src="<?php echo HIKAMARKET_IMAGES; ?>icon-16/plus.png" alt="" style="vertical-align:middle;"/> <?php echo JText::_('HIKA_ADD_RESTRICTION'); ?></button>
<?php
		}
...
window.pluginMgr.addRestriction = function() {
	...
	for(var j = container.childNodes.length - 1; j >= 0; j--) {
		...
		if(e.nodeType && e.className && e.className == key)
			e.style.display = '';
	}
