...
class pluginmarketViewpluginmarket extends hikamarketView {
	...
	public function listing($tpl = null) {
		...
		$type = JRequest::getCmd('plugin_type', 'payment');
		...
		if($vendor->vendor_id > 1) {
			$filters[] = 'plugin.'.$filter_type.'_vendor_id = ' . (int)$vendor->vendor_id;
		} else {
			...
			if($pageInfo->filter->vendors >= 0) {
				if($pageInfo->filter->vendors > 1)
					$filters[] = 'plugin.'.$filter_type.'_vendor_id = '.(int)$pageInfo->filter->vendors;
				else
					$filters[] = 'plugin.'.$filter_type.'_vendor_id <= 1';
			}
		}
		...
		switch($type) {
			case 'payment':
				$pluginInterfaceClass = hikamarket::get('shop.class.payment');
				break;
			case 'shipping':
				$pluginInterfaceClass = hikamarket::get('shop.class.shipping');
				break;
			case 'generic':
			default:
	...
	public function form($tpl = null) {
		...
		$subtask = JRequest::getCmd('subtask', 'edit');
	...
	public function add($tpl = null) {
		...
		if(!HIKASHOP_J16) {
			$query_select = array('id', 'published', 'name', 'element');
			$query_table = 'plugins';
			$query_filters = array(
				'`folder` = '.$db->Quote($plugin_group)
			);
			if((int)$pageInfo->filter->publish >= 0)
				$query_filters[] = 'published = ' . (int)$pageInfo->filter->publish;
			if(!empty($pageInfo->search))
				$query_filters[] = 'name LIKE \'%' . $db->getEscape(JString::strtolower($pageInfo->search)) . '%\'';

			$query_order = 'published DESC, name ASC, ordering ASC';
		} else {
			...
			if(!empty($pageInfo->search)) {
				if(!HIKASHOP_J30)
					...
					$query_filters[] = 'name LIKE \'%' . $db->escape(JString::strtolower($pageInfo->search)) . '%\'';
			}
	...
	protected function processConfig($configs, $type, $paramsType, $configData, $id = '', $checkDisplay = false) {
		...
		foreach($configs as $key => $config) {
			...
			$classname = empty($config['category']) ? '' : 'hikamarket_'.$type.'_cat_'.$config['category'];
			$style = empty($config['hidden']) ? '' : ' style="display:none;"';

			if(!empty($config['display']))
				$style .= ' id="hikamarket_field_{TYPE}_'.$paramsTypeKey.$key.'"';

			$html .= '<dt class="'.$classname.'"'.str_replace('{TYPE}', 'title', $style).'><label for="data_'.$type.'_'.$paramsTypeKey.$key.'">'.$label.'</label></dt>'."\r\n".
					'<dd class="'.$classname.'"'.str_replace('{TYPE}', 'value', $style).'>';
			...
			switch($config['type']) {
				...
				case 'wysiwyg':
					...
					$html .= $this->editorHelper->display();
					break;
				...
				case 'boolean':
					...
					$html .= JHTML::_('hikaselect.booleanlist', 'data['.$type.']'.$paramsTypeForm.'['.$key.']' , 'onchange="'.$jsEvent.'"', $data);
					break;
				...
				case 'radio':
					...
					$html .= JHTML::_('hikaselect.radiolist', $values, 'data['.$type.']'.$paramsTypeForm.'['.$key.']' , 'onchange="'.$jsEvent.'" class="inputbox" size="1"', 'value', 'text', $data);
					break;
				...
				case 'currencies':
					if(empty($this->currenciesType))
						$this->currenciesType = hikamarket::get('shop.type.currency');
					$html .= $this->currenciesType->display('data['.$type.']['.$paramsTypeForm.']['.$key.']', $data, 'multiple="multiple" size="3"');
					break;
				...
				default:
					$ret = false;
					if(!empty($this->plugin) && method_exists($this->plugin, 'displayConfigField'))
